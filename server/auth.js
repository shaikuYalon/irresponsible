...
router.post('/login', (req, res) => {
    ...
    const sql = `SELECT user_id, username, role FROM Users WHERE username = ? AND password = ?`; // הוספת עמודת role
    ...
    connection.query(sql, values, (err, results) => {
        ...
        if (results.length > 0) {
            ...
            res.json({ message: 'Login successful', user }); // החזרת userId, username, ו-role
        } else {
